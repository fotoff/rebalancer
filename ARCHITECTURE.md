...
> **RPC:** Alchemy обслуживает `eth_call`, но `eth_getLogs` не поддерживает; публичный
> `mainnet.base.org` умеет логи, но ограничен 10k блоков за вызов и жёстко лимитирован.
> Поэтому чтение состояния идёт через Alchemy, а события — через публичный узел
> кусками, с честно объявленной глубиной окна.
...
|---|---|
| `/agent-vault` | UI: деплой vault'а, пополнение (ETH оборачивается в WETH), выдача/отзыв прав |
| `/agents` | Публичные доки для разработчиков агентов |
| `/api/agent/activity` | События `AgentTraded` с блокчейна — окно ~2 суток, `truncated` в ответе |
| `packages/agent-demo` | Reference-агент: canTrade → сигнал → LI.FI → agentTrade. Без `--execute` не броадкастит |

Пользователь → **один** AgentVault; vault → **много** агентов; агент → **много** vault'ов
у разных пользователей. Агенты в одном vault'е изолированы друг от друга.

**Discovery не решён:** какие vault'ы авторизовали агента, узнать можно только перебором
фабрики или off-band. Событие `AgentPermissionSet` индексировано по агенту специально
под будущий индексер.
